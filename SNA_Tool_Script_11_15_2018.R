...
<cut.off])
  ...
  if(keyplayer==TRUE){
    rmarkdown::render("SNA_Rmarkdown_Output_with_Keyplayers.Rmd","word_document")}
   else{
    lines <- readLines("Rmarkdown_test.Rmd")
    writeLines(purrr::map_chr(1:134, ~ lines[.]), "SNA_Rmarkdown_Output_without_Keyplayers.Rmd")
    rmarkdown::render("SNA_Rmarkdown_Output_without_Keyplayers.Rmd","word_document")
   }
